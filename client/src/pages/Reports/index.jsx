...
function ReportsPage() {
  ...
  return (
    <div className='space-y-4 h-full'>
      <Header
        title={'Rapor'}
        description={'Raporlarınızı buradan inceleyebilirsiniz ve indirebilirsiniz.'}
      />
      <div className='space-x-3 grid grid-cols-12 h-full'>
        {/* menü şeklinde rapoların tip listesi */}

        <div className='flex flex-col space-y-1 col-span-2'>
          <ul className='mr-4 list-none flex flex-col '>
            {reports.map((report, index) => (
              <li role='presentation' key={index} className='flex-grow  hover:cursor-pointer'>
                <div
                  className={`${
                    selectedReport === report.src ? 'border-b border-blue-600' : ''
